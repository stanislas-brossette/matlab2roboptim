// compile command: 
// g++ -ggdb3 -I/usr/include/log4cxx `pkg-config --cflags roboptim-core` src/generatedFiles/planarRobot4Direct.cc `pkg-config --libs roboptim-core` -o bin/planarRobot4Direct
#include <iostream>
...
template <typename T>
class planarRobot4Direct : public roboptim::GenericDifferentiableFunction<T>
{
public:
  ROBOPTIM_DIFFERENTIABLE_FUNCTION_FWD_TYPEDEFS_
  (roboptim::GenericDifferentiableFunction<T>);
  
  explicit planarRobot4Direct (const double& EE_1_1,
...
template <typename T>
planarRobot4Direct<T>::planarRobot4Direct (const double& EE_1_1,
			 const double& EE_1_2) throw ()
  : roboptim::GenericDifferentiableFunction<T>
    (4, 2, "Direct EE position constraints planarRobot4"),
    EE_1_1 (EE_1_1),
		EE_1_2 (EE_1_2)
{}
...
template <typename T>
void
planarRobot4Direct<T>::impl_compute (result_t& result, const argument_t& x)
  const throw ()
{
...
template <typename T>
void
planarRobot4Direct<T>::impl_gradient (gradient_t& grad, const argument_t& x, size_type id)
  const throw ()
{
  const double& q_01 = x[0];
	const double& q_02 = x[1];
...
int main ()
{
  ...
  roboptim::Function::vector_t offset (4);
